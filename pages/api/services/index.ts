...
const getPartnerServices: NextApiHandler<GetResponse> = async (req, res) => {
  ...
  if (id) {
    ...
    const service = await getServiceById({ collection: collection, id: id, partnerId: partnerId || '' })
    if (!service) throw new createHttpError.NotFound('Serviço não encontrado.')
    return res.status(200).json({ data: service })
  }

  const services = await getServices({ collection: collection, partnerId: partnerId || '' })
...
const editProduct: NextApiHandler<PutResponse> = async (req, res) => {
  ...
  const updateResponse = await updateService({ collection: collection, id: id, changes: changes, partnerId: partnerId || '' })
