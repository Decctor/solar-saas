...
function StructureInfo({
  ...
  function validateFields() {
    ...
    if (
      ...
      toast.error(
        "Por favor, preencha o responsável pela construção/adequações de estrutura."
      );
      return false;
    }
    if (
      requestInfo.responsavelEstrutura != "NÃO SE APLICA" &&
      requestInfo.formaPagamentoEstrutura == "NÃO DEFINIDO"
    ) {
      ...
      <div className="flex w-full grow flex-col items-center gap-2">
        <div className="grid w-full grid-cols-1 grid-rows-2 gap-2 lg:grid-cols-2 lg:grid-rows-1">
          ...
          <SelectInput
            ...
            selectedItemLabel="NÃO DEFINIDO"
            onReset={() =>
              setRequestInfo((prev) => ({ ...prev, materialEstrutura: null }))
            }
            width="100%"
          />
        </div>
        <div className="grid w-full grid-cols-1 grid-rows-2 gap-2 lg:grid-cols-2 lg:grid-rows-1">
          ...
          <SelectInput
            ...
            handleChange={(value) =>
              setRequestInfo({ ...requestInfo, responsavelEstrutura: value })
            }
            selectedItemLabel="NÃO DEFINIDO"
            onReset={() =>
              setRequestInfo((prev) => ({
