...
function NewClient({ session, partnerId, closeModal }: NewClientModalProps) {
      ...
      <div className="fixed left-[50%] top-[50%] z-[100] h-[80%] w-[93%] translate-x-[-50%] translate-y-[-50%] rounded-md bg-[#fff] p-[10px]">
        <div className="flex h-full flex-col">
          <div className="flex flex-col items-center justify-between border-b border-gray-200 px-2 pb-2 text-lg lg:flex-row">
            <h3 className="text-xl font-bold text-[#353432] dark:text-white ">NOVO CLIENTE</h3>
            <button
              onClick={closeModal}
              type="button"
          ...
          <div className="my-2 flex w-full items-center justify-end px-4">
            <button
              disabled={isPending}
              // @ts-ignore
              onClick={() => handleCreateClient({ info: clientInfo })}
              className="rounded bg-green-500 px-4 py-1 text-sm font-medium text-white duration-300 ease-in-out disabled:bg-gray-500 disabled:text-black enabled:hover:bg-green-700"
            >
              CRIAR
            </button>
          </div>
