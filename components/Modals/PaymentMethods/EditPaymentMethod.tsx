...
function EditPaymentMethod({ paymentMethodId, session, closeModal }: EditPaymentMethodProps) {
  ...
  const { mutate: handleEditPaymentMethod } = useMutationWithFeedback({
    mutationKey: ['edit-payment-method', paymentMethodId],
    mutationFn: editPaymentMethod,
    queryClient: queryClient,
    affectedQueryKey: ['payment-methods'],
    callbackFn: async () => await queryClient.invalidateQueries({ queryKey: ['payment-method-by-id', paymentMethodId] }),
  })
  useEffect(() => {
    if (method) setInfoHolder(method)
  }, [method])
  return (
      ...
      <div className="fixed left-[50%] top-[50%] z-[100] h-[80%] w-[90%] translate-x-[-50%] translate-y-[-50%] rounded-md bg-[#fff] p-[10px] lg:w-[70%]">
        <div className="flex h-full flex-col font-Inter">
          <div className="flex flex-col items-center justify-between border-b border-gray-200 px-2 pb-2 text-lg lg:flex-row">
            <h3 className="text-xl font-bold text-[#353432] dark:text-white ">EDITAR MÉTODO DE PAGAMENTO</h3>
            <button
              onClick={() => closeModal()}
              type="button"
          ...
          {isError ? <ErrorComponent msg="Oops, houve um erro ao buscar método de pagamento." /> : null}
          {isSuccess ? (
            <>
